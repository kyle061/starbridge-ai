...
# Match the pnpm version used by CI.
...
RUN --mount=type=cache,id=sub2api-gomod,target=/go/pkg/mod \
    ...
    -tags embed \
    -ldflags="-s -w -X main.Version=${VERSION_VALUE} -X main.Commit=${COMMIT} -X main.Date=${DATE_VALUE} -X main.BuildType=source" \
    -trimpath \
    -o /app/sub2api \
    ./cmd/server
...
# Labels
LABEL maintainer="kyle061 <github.com/kyle061>"
LABEL description="Starbridge AI - multi-provider AI gateway, based on Sub2API"
LABEL org.opencontainers.image.source="https://github.com/kyle061/starbridge-ai"
