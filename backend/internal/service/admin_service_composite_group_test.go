...
// 独立 CN 分组使用各自的公开模型目录；composite 支持不得改变独立分组的候选语义。
func TestAdminService_CNProviderModelsListCandidatesUseProviderCatalog(t *testing.T) {
	for _, platform := range []string{PlatformKimi, PlatformZhipu, PlatformDeepseek, PlatformMiniMax} {
		require.Equal(t, modelcatalog.ModelsForPlatform(platform), defaultModelsListCandidateIDs(platform), "platform=%s", platform)
	}
}
