...
func (s *FrontendServer) serveIndexHTML(c *gin.Context) {
	...
	// SPA routes share index.html, so provide a path-aware crawler directive at
	// the HTTP layer. This protects private routes even when JavaScript is off.
	c.Header("X-Robots-Tag", robotsDirectiveForPath(c.Request.URL.Path))
...
func robotsDirectiveForPath(path string) string {
	cleanPath := strings.TrimSuffix(path, "/")
	if cleanPath == "" || cleanPath == "/" || cleanPath == "/home" || cleanPath == "/model-plaza" || strings.HasPrefix(cleanPath, "/legal/") {
		return "index, follow"
	}
...
func (s *FrontendServer) injectSettings(settingsJSON []byte) []byte {
	...
	result = injectSiteDescription(result, settingsJSON)

	return result
}

// injectSiteDescription replaces the static SEO description with the configured
// public site subtitle before the SPA loads. This lets crawlers and link
// previews see the operator's current description without executing JavaScript.
func injectSiteDescription(html, settingsJSON []byte) []byte {
	var cfg struct {
	...
	return replaceMetaTag(html, `name="description"`, []byte(`<meta name="description" content="`+htmlpkg.EscapeString(description)+`" />`))
}

func replaceMetaTag(html []byte, marker string, replacement []byte) []byte {
	markerIndex := bytes.Index(html, []byte(marker))
	if markerIndex == -1 {
		return html
	}
	tagStart := bytes.LastIndex(html[:markerIndex], []byte("<meta"))
	if tagStart == -1 {
		return html
	}
	tagEndOffset := bytes.IndexByte(html[markerIndex:], '>')
	if tagEndOffset == -1 {
		return html
	}
	tagEnd := markerIndex + tagEndOffset + 1
	result := make([]byte, 0, len(html)-tagEnd+tagStart+len(replacement))
...
func ServeEmbeddedFrontend() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		c.Header("X-Robots-Tag", robotsDirectiveForPath(path))
