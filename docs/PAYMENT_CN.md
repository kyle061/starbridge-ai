...
Starbridge AI 内置支付系统，支持用户自助充值，无需部署独立的支付服务。
...
> 支付渠道的安全性、稳定性、费率、结算周期及合规性请由运营方自行核验。Starbridge AI 不对任何第三方支付服务商做担保或背书，建议优先使用运营方已签约并完成审核的支付渠道。
...
如果你之前使用其他外部支付系统，现在可以迁移到 Starbridge AI 内置支付：

### 主要差异

| 对比项 | 外部支付系统 | Starbridge AI 内置支付 |
|--------|-----------|---------|
| 部署方式 | 独立服务 | 内置于 Starbridge AI，无需额外部署 |
| 支付方式 | EasyPay、支付宝、微信、Stripe | 相同 |
| 配置方式 | 环境变量 + 独立管理后台 | Starbridge AI 管理后台内统一配置 |
| 充值对接 | 通过 Admin API 回调 | 内部直接处理，更可靠 |
| 订阅套餐 | 支持 | 暂不支持（计划中） |
| 订单管理 | 独立管理界面 | 集成在 Starbridge AI 管理后台 |

### 迁移步骤

1. 在 Starbridge AI 管理后台启用支付并配置服务商（使用相同的支付凭证）
2. 更新 Webhook 回调地址为 Starbridge AI 的回调地址
3. 确认新订单通过内置支付正常处理
4. 停用原外部支付服务

> **注意**：外部支付系统中的历史订单数据不会自动迁移。建议保留原系统一段时间以便查询历史记录。
