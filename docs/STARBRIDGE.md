...
1. 管理员在 **支付管理** 配置人民币收款渠道（支付宝、微信等）的商户信息与回调地址，并启用渠道。余额充值倍率为 `2`、附加手续费为 `0`、最低充值金额为 `0.5`；渠道自身的最低金额仍需满足。支付入口默认开启，未配置渠道时不能完成付款。
2. 用户首次支付成功、余额入账后，系统自动生成一个长期有效的 Key，优先绑定用户可用的余额计费分组。没有可用分组时 Key 暂时不绑定分组，需要管理员先配置线路，再由用户选择分组。
3. 用户的所有 Key 共用账户美元余额。服务端每次鉴权、排队后的计费检查及长连接的新一轮调用都会核对数据库；余额小于或等于零时暂停新的模型请求。余额状态查询与已完成图片任务的结果读取仍可使用。
4. 续费到账且余额恢复为正后继续使用原 Key，无须换 Key。请求已开始后仍会完成并结算，并发请求可能产生少量欠款，充值先抵扣欠款。

只有完成的余额支付订单（或仍有未退款额度的部分退款订单）能解锁首次使用。注册赠送、后台直接加余额及普通兑换码不替代首次在线支付；管理员账号保留管理和测试能力。Key 被手动禁用或设置了独立用量上限时，相应限制仍然生效。

迁移 `242` 会为已有站点启用支付入口、设置上述价格、关闭用户订阅购买入口，并将普通用户已有 Key 的时间期限清除。迁移后管理员仍可调整价格。部署使用标准计费模式，`billing.require_balance_purchase` 默认开启；开启时不能使用跳过计费的 `simple` 模式。
