...
6. 在密钥列表点击 **使用密钥 → Codex CLI**，复制生成的单文件 `~/.codex/config.toml`。默认配置包含 `model = "gpt-6-astra"`、`model_provider = "starbridaeai"`、`name = "starbridaeai"`、`model_reasoning_effort = "xhigh"`、`wire_api = "responses"`、`supports_websockets = true`、`requires_openai_auth = true` 和当前站点 `/v1` 地址。保存后重启 Codex 即可；也可以切换到环境变量模式。
