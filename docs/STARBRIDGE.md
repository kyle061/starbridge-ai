...
使用资格以账户实际余额为准，无须在线支付订单。管理员账号保留管理和测试能力。Key 被手动禁用或设置了独立用量上限时，相应限制仍然生效。用户创建密钥只填写名称、分组，密钥默认长期有效；编辑及批量编辑不会改动额度、限速或有效期。管理员可在 **用户管理 → 用户密钥 → 编辑** 设置单个 Key 的额度和 5 小时/1 天/7 天用量上限；分组倍率和用户请求限速在原有管理端配置，用户界面不展示倍率及限速控件。
