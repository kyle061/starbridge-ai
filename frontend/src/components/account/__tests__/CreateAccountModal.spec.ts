...
describe('CreateAccountModal OpenAI long-context billing', () => {
  ...
  it.each([
    ['openai', 'https://api.openai.com', 'force_responses'],
    ['deepseek', 'https://api.deepseek.com', 'force_chat_completions'],
    ['siliconflow', 'https://api.siliconflow.cn/v1', 'force_chat_completions'],
    ['openrouter', 'https://openrouter.ai/api/v1', 'force_chat_completions'],
    ['ollama', 'http://host.docker.internal:11434/v1', 'force_chat_completions'],
  ])('submits %s through existing account credentials and protocol fields', async (id, baseUrl, mode) => {
    const wrapper = mountModal()
    await selectButtonByText(wrapper, 'OpenAI')
    await selectButtonByText(wrapper, 'API Key')
    await wrapper.get('form#create-account-form input[type="text"]').setValue('my upstream')
    await wrapper.get('form#create-account-form input[type="password"]').setValue('test-provider-key')
    await wrapper.get(`[data-testid="provider-preset-${id}"]`).trigger('click')
    expect(createAccountMock).not.toHaveBeenCalled()
    await wrapper.get('form#create-account-form').trigger('submit.prevent')
    await flushPromises()
    expect(createAccountMock).toHaveBeenCalledTimes(1)
    expect(createAccountMock.mock.calls[0][0]).toMatchObject({
      name: 'my upstream', platform: 'openai', type: 'apikey',
      credentials: { base_url: baseUrl, api_key: 'test-provider-key' },
