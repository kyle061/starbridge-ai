        ...
        <aside class="space-y-3 rounded-lg border border-gray-200 bg-gray-50 p-4 text-sm dark:border-dark-700 dark:bg-dark-900/60">
          <div>
            ...
            <p class="mt-1 break-all font-mono text-gray-900 dark:text-white">
              {{ complianceStore.status?.version || 'v2026.09.17' }}
            </p>
          </div>
          <a
            :href="documentUrl"
...
const documentUrl = computed(() => {
  if (getLocale() === 'zh') {
    return complianceStore.status?.document_url_zh || '/legal/admin-compliance'
  }
  return complianceStore.status?.document_url_en || '/legal/admin-compliance'
})
