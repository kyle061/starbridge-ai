...
function mountTable(models: PlazaModel[]) {
  return mount(PlazaModelPricingTable, { props: { models } })
}

describe('PlazaModelPricingTable', () => {
  it('只展示后端返回的实际价格，不展示任何倍率字段', () => {
    const wrapper = mountTable([tokenModel({ name: 'gpt-6-astra' })])
    expect(wrapper.text()).toContain('$3.00')
    expect(wrapper.text()).toContain('$15.00')
    expect(wrapper.text()).not.toMatch(/\d+(?:\.\d+)?x/)
    expect(wrapper.find('.line-through').exists()).toBe(false)
  })
  it('token 价格统一按后端实际价显示并保底 2 位小数', () => {
  ...
  it('官方参考价与用户实际价格分栏展示', () => {
    const wrapper = mountTable([tokenModel({
      pricing: {
        ...
        input_price: 18e-6,
        output_price: 90e-6,
        cache_write_price: 22.5e-6,
        cache_read_price: 1.8e-6
      }
    })])
    const text = wrapper.text()
  ...
  it('模型按官方输出价从高到低排序,无官方价的排最后', () => {
    ...
    const wrapper = mountTable([cheap, noOfficial, expensive])
  ...
  it('官方输出价相同时按模型名降序(新版本号在前)', () => {
    ...
    const wrapper = mountTable([older, newer])
    const names = wrapper.findAll('tbody tr').map((tr) => tr.find('td').text())
  ...
  it('按图片/按次计费的模型沉到末尾,不与 token 模型按官方价混排', () => {
    ...
    const wrapper = mountTable([pricier, image, cheaper])
    const names = wrapper.findAll('tbody tr').map((tr) => tr.find('td').text())
  ...
  it('官方价包含 1h 缓存写入价;official_pricing 为 null 时官方三列显示 -', () => {
    const withOfficial = mountTable([tokenModel()])
    expect(withOfficial.text()).toContain('$6.00')
    expect(withOfficial.text()).toContain('(1h')

    const withoutOfficial = mountTable([tokenModel({ official_pricing: null })])
  ...
  it('仅配置区间倍率时按基础价格展示 input/output/cache 各档价格', () => {
    ...
    const cells = mountTable([model]).findAll('tbody td')
...
describe('PlazaModelPricingTable 长上下文阶梯', () => {
  ...
  it('实付缓存列按档分行并乘倍率,每档一行与输入/输出列对齐;档位标签只在输入列', () => {
    ...
    expect(rows[0].text()).toContain('$6.25')
  ...
  it('整单计价的档位标签带 tooltip;边际计价在模型名旁加徽章并换用边际说明', () => {
    const whole = mountTable([ladderModel()])
    ...
    const marginal = mountTable([ladderModel({ long_context_basis: 'marginal' })])
