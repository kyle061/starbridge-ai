export default {
    accounts: {
      providerPresets: {
        ...
        openaiOAuthHint: "ChatGPT Plus / Pro 必须选择 OAuth 登录，才能使用该账号的 Codex 订阅额度；OpenAI Platform API Key 会单独按 API 用量计费。可重复添加多个 Plus / Pro 或 API Key 账号到同一分组，当前账号限流或额度耗尽时会自动切换。"
      },
      title: '账号管理',
      description: '管理 AI 平台账号和 Cookie',
      accountPoolTitle: '多账号自动切换已启用',
      accountPoolDescription: '可重复添加 OpenAI、Claude、Gemini、Grok 和兼容平台账号；同一平台分组可混合 OAuth 与 API Key。账号额度耗尽、429、认证失效或临时不可用时，系统会自动切换到同组健康账号。一个密钥访问多个平台时，请使用组合分组和模型路由。',
      accountPoolAddAnother: '继续添加账号',
      createAccount: '添加账号',
      autoRefresh: '自动刷新',
      ...
      usageWindowsHint: '“5h / 7d”是上游账号（如 OpenAI ChatGPT、Claude）官方的滚动用量窗口限制，由上游对账号设定，并非 Starbridge AI 配置，也与你映射的模型无关。窗口滚动到期后用量会自动重置，无法在 Starbridge AI 端解除该限制。',
      ollamaCloud: {
      ...
      upstreamBilling: {
        trustWarning: '此倍率由上游站点针对当前 API Key 自行声明。Starbridge AI 无法验证该值是否与实际扣费一致；上游站点或中间代理可能返回伪造、过期或被篡改的数据。请结合账单、余额变化和实际用量自行核验。',
      ...
      types: {
        ...
        responsesApi: 'OpenAI / 兼容 API Key',
      ...
      usageWindow: {
        ...
        grokRequests: '请求',
        grokTokens: 'Token',
        grokFreeQuota24hHint: '按 Starbridge AI 近 24 小时本地 Token 用量估算（上限 {limit}）',
      ...
      upstreamRequestIdHeaderHelp: {
        ...
        examplesTitle: '常见取值',
        starbridgeNote: '对应 Starbridge AI 用量明细的请求ID列',
      ...
      grok: {
        ...
        ttsTextLabel: 'TTS 文本',
        ttsTextPlaceholder: '例如：Hello from Starbridge AI connectivity test.',
      ...
      poolModeInfo:
        '启用后，上游 429/403/401 错误将自动重试而不标记账号限流或错误，适用于上游指向另一个兼容中转站实例的场景。',
      ...
      oauth: {
        ...
        callbackRecognized: '已识别回调链接，可以完成授权。',
        ...
        // OpenAI specific
        openai: {
          title: 'ChatGPT Plus / Pro 账号授权',
          followSteps: '登录 ChatGPT 账号以使用该账号的 Codex 订阅额度：',
          device: {
            label: "设备码登录（推荐手机使用）",
            description: "获取设备码后，在 OpenAI 官方页面登录并输入设备码。完成授权后切回当前页面，本站会自动接收结果并绑定账号。",
            enableHint: "首次使用请在 ChatGPT → 设置 → 安全中开启设备码登录。若账号不支持，可选择手动回调登录或导入 Codex 会话。",
            returnAddress: "绑定结果接收页面（当前站点）",
            start: "获取登录设备码",
            starting: "正在获取设备码…",
            codeLabel: "一次性设备码（15 分钟内有效）",
            open: "打开 OpenAI 登录页面",
            waiting: "等待授权，请完成后切回本页面。",
            authorized: "授权成功，正在保存账号。保存失败可重试。",
            save: "重试保存账号",
            restart: "重新获取设备码",
            expired: "设备码已过期或会话失效，请重新获取。",
            retrying: "暂时无法查询授权结果，正在重试…",
            failed: "获取设备码失败。请检查代理连接，并确认 ChatGPT 安全设置已开启设备码登录。",
          },
          chatgptLogin: '手动回调登录（Plus / Pro）',
          step1GenerateUrl: '点击下方按钮生成授权链接',
          generateAuthUrl: '生成授权链接',
          openAuthorizationPage: '打开 ChatGPT 登录页面',
          step2OpenUrl: '在浏览器中打开链接并完成授权',
          openUrlDesc: '点击上方按钮，在新标签页登录需要绑定的 ChatGPT Plus / Pro 账号并同意授权。每次可绑定一个账号，完成后可继续添加其他账号。',
          importantNotice:
            'OpenAI 官方 Codex OAuth 客户端要求使用本地回调 {callbackUrl}。授权后即使页面提示无法连接，也请复制地址栏里的完整链接，回到星桥粘贴到第 3 步。手机端请长按地址栏并选择复制。',
        ...
        gemini: {
          ...
          aiStudioNotConfiguredTip:
            'AI Studio OAuth 未配置：请先设置 GEMINI_OAUTH_CLIENT_ID / GEMINI_OAUTH_CLIENT_SECRET，并在 Google OAuth Client 添加当前站点的回调地址（通常为 https://starbridaeai.top/auth/callback；Consent Screen scopes 需包含 https://www.googleapis.com/auth/generative-language.retriever）',
          aiStudioNotConfigured:
            'AI Studio OAuth 未配置：请先设置 GEMINI_OAUTH_CLIENT_ID / GEMINI_OAUTH_CLIENT_SECRET，并在 Google OAuth Client 添加当前站点的 /auth/callback 回调地址'
        },
        ...
        antigravity: {
          ...
          openUrlDesc: '请在新标签页中打开授权链接，登录您的 Google 账户并授权。',
          importantNotice:
            '重要提示：授权后页面可能会加载较长时间，请耐心等待。当浏览器跳转到授权流程提供的 callback 页面时，表示授权已完成。',
          step3EnterCode: '输入授权链接或 Code',
          authCodeDesc:
            '授权完成后，当页面地址变为授权流程提供的 callback?code=... 时：',
          authCode: '授权链接或 Code',
          authCodePlaceholder:
            '方式1：复制完整的 callback 链接（包含 code 参数）\n方式2：仅复制 code 参数的值',
