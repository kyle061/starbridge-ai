export default {
    ...
    channels: {
      ...
      form: {
        ...
        modelPricing: '模型定价（原始渠道数据）',
        rawPricingHint: '管理员视图：这里显示并编辑渠道原始定价，不含客户端全局计费倍率。',
        models: '模型列表',
        modelsPlaceholder: '输入完整模型名后按回车添加',
        modelInputHint: '按回车添加，支持粘贴批量导入',
        billingMode: '计费模式',
        defaultPrices: '渠道原始价格（未命中区间时使用）',
        inputPrice: '输入',
        outputPrice: '输出',
        cacheWritePrice: '缓存写入',
        cacheWrite5mPrice: '缓存写入（5m）',
    ...
    riskControl: {
      ...
      baseUrlRequired: '请填写 OpenAI Base URL',
      baseUrlInvalid: 'OpenAI Base URL 必须是有效的 HTTP/HTTPS 地址',
      modelRequired: '请填写审核模型名',
      invalidRange: '{field} 必须在 {min} 到 {max} 范围内',
      invalidThreshold: '风险阈值必须在 0% 到 100% 之间',
      groupScopeRequired: '关闭全部分组后，至少选择 1 个审计分组',
      preBlockApiKeyRequired: '启用内容审计前，请至少配置 1 个可用的审核 API Key',
    ...
    subscriptions: {
      ...
      quotaHint: '额度由本订阅的 API 调用扣除，用完后停止调用；不会每天重置。',
      quotaEditHint: '修改的是总额度，会保留已用额度与到期时间。原订阅未设置额度时，从保存后开始累计。',
      quotaRequired: '请输入大于 0 的订阅额度和消耗倍率',
      quotaBelowUsage: '总额度不能小于已消耗的订阅额度',
      subscriptionMultiplier: '本订阅消耗倍率',
      subscriptionMultiplierHint: '只影响当前这条订阅；不会修改分组默认倍率、套餐默认值或其他订阅。此值是本订阅的最终扣费倍率。',
      assignmentMultiplierHint: '分配时可针对这条订阅单独设置；保存后只作用于该订阅，不会改变分组默认倍率。',
      multiplierRequired: '请输入大于 0 的订阅消耗倍率',
      planTemplate: '套用套餐配置',
      customQuota: '自定义额度',
      planTemplateHint: '选择套餐后自动填入有效期、额度和后台计费参数，也可手动调整。',
      ...
      guide: {
        title: '订阅管理教程',
        subtitle: '先分配有效期和总额度，再让用户创建对应分组的 API 密钥。总额度不会每天重置，日/周/月限额可按需额外设置。',
        overview: '使用方式：分配订阅时套用套餐或填写总额度与有效期；用户在「API 密钥」创建密钥并选择相同订阅分组，随后通过「使用密钥」配置客户端。调用后可在本页及「我的订阅」查看额度消耗。',
        showGuide: '使用指南',
        step1: {
          title: '创建订阅分组',
          line1: '前往「分组管理」页面，点击「创建分组」',
          line2: '将计费类型设为「订阅」，按套餐设置可用模型；日/周/月额度限制可按需配置',
          line3: '关联可用的上游账号，保存分组并确保状态为「正常」',
          link: '前往分组管理'
        },
        step2: {
          title: '分配订阅给用户',
          ...
          line3: '选择订阅分组，套用套餐配置或填写总额度和有效期，确认后点击「分配」'
        },
        step3: {
          title: '管理已有订阅'
        },
        step4: {
          title: '告知用户如何使用',
          line1: '用户进入「API 密钥」创建密钥，选择与该订阅相同的分组；分配订阅不会自动生成密钥。',
          line2: '点击「使用密钥」复制客户端配置，使用本站接口地址及该密钥，选择套餐支持的模型。',
          line3: '发送一条消息后，在「使用记录」查看调用，再刷新「我的订阅」查看额度。订阅到期或额度耗尽后停止调用，不会自动改扣余额。'
        },
        actions: {
          quotaDesc: '修改订阅总额度，保留已消耗额度和到期时间；为旧订阅补充额度后开始累计',
          adjust: '调整',
          adjustDesc: '延长或缩短订阅有效期',
          resetQuota: '重置配额',
          resetQuotaDesc: '只将日/周/月窗口用量归零，不会恢复订阅总额度',
          revoke: '撤销',
