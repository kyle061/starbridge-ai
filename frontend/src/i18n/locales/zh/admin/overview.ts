export default {
    ...
    backup: {
      ...
      r2Guide: {
        ...
        step1: {
          ...
          line2: '点击「创建存储桶」，输入名称（如 starbridge-backups），选择区域',
          line3: '点击创建完成'
        },
        step2: {
          title: '创建 API 令牌',
    ...
    groups: {
      ...
      form: {
        ...
        rateMultiplierLabel: '分组默认倍率',
        rateMultiplierHint: '用于该分组的默认计费；订阅分配时可以单独设置本订阅倍率。1.0 = 标准费率，0.5 = 半价，2.0 = 双倍',
        rpmLimit: '每分钟请求数 (RPM)',
        rpmLimitPlaceholder: '0 表示不限制',
        rpmLimitHint: '每用户在本分组每分钟最大请求数，0 = 不限制；一旦设置即接管该用户的限流（覆盖用户级 rpm_limit）',
        concurrencyLimit: '分组总并发上限',
        concurrencyLimitPlaceholder: '1000',
        concurrencyLimitHint: '独立于单账号并发。实际容量取该值与可调度账号并发总和的较小值；填 0 表示按账号并发总和。',
        ...
        reasoningEffortModelPlaceholder: '留空则全部 / gpt / gpt-5.6-sol',
      ...
      compositeRoutes: {
        ...
        routes: '已保存路由',
        failoverHint: '同一公开模型可添加多条路由。优先级数字越小越先使用，当前线路没有可用账号时会自动切换到下一条兼容线路。',
      ...
      openaiMessages: {
        ...
        sonnetModelPlaceholder: '例如: gpt-5.6-terra',
        haikuModel: 'Haiku 映射模型',
        haikuModelPlaceholder: '例如: gpt-5.6-luna',
      ...
      openaiLive: {
        title: 'OpenAI Live',
        allow: '允许访问 Live',
        hint: '启用后，此 OpenAI 分组的 API Key 可以创建并控制 Live 语音会话。默认关闭。运行 Starbridge AI 的服务端必须是 Apple Silicon Mac，并安装官方 ChatGPT App；客户端平台不受限制。',
        unsupportedTitle: '当前服务端不支持 Live',
        unsupportedMessage: '当前 Starbridge AI 服务端无法生成 Live 所需的设备证明，即使开启也不能使用。是否仍然开启？',
        enableAnyway: '仍然开启'
      },
      openaiFast: {
