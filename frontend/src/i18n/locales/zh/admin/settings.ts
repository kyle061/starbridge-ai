    ...
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          freezeHoursDesc: '新产生的返利在冻结期内无法转入账户余额。0 = 不冻结。',
      ...
      linuxdo: {
        ...
        description: '配置 LinuxDo Connect OAuth，用于 Starbridge AI 用户登录',
      ...
      dingtalk: {
        title: '钉钉登录',
        description: '配置钉钉 OAuth，用于 Starbridge AI 用户登录',
      ...
      scheduling: {
        ...
        accountSchedulingThresholdsDescription: '当账号当前原生用量窗口（OpenAI Codex/Anthropic 会话，或 Grok 请求/Token 利用率）达到该百分比时，Starbridge AI 会临时将其移出调度，直到窗口重置。填 100 表示禁用。',
      ...
      upstreamBillingProbe: {
        title: '上游倍率自动探测',
        description: '定期获取 API Key 账号所连接上游兼容中转站声明的计费倍率；只有另行开启“同步上游声明倍率”的账号才会更新账号倍率。',
        enabled: '启用全局自动探测',
      ...
      site: {
        ...
        siteSubtitlePlaceholder: '多模型 AI API 中转站',
        apiBaseUrl: 'API 端点地址',
        ...
        contactInfo: '微信联系方式',
        contactInfoPlaceholder: '例如：微信：your_wechat_id',
        contactInfoHint: '填写微信号（可加“微信：”前缀），所有联系入口统一展示并支持一键复制；也兼容邮箱和 HTTPS 联系页。',
        docUrl: '文档链接',
      ...
      payment: {
        ...
        validationEasyPaySigningRequired: '易支付请填写完整的 RSA 私钥和公钥，或填写传统协议的商户密钥（Key）',
        ...
        easypayCustomMethodsHint: '添加当前易支付服务商额外支持的支付方式。支付方式会记录到 Starbridge AI 订单中，上游 type 会作为易支付 type 参数提交。',
      ...
      resend: {
        ...
        apiKeyPlaceholder: '请填写真实的 re_ 开头密钥',
        apiKeyHint: '请将示例 re_xxxxxxxxx 替换为 Resend 控制台创建的真实密钥。',
        apiKeyConfiguredPlaceholder: '********',
        apiKeyConfiguredHint: '密钥已配置，留空可保留当前值。',
        fromEmail: '备用发件人邮箱',
