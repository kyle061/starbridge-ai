export default {
  ...
  keys: {
    ...
    renewBalance: '兑换码续充',
    prepaidPaymentSuccess: '美元额度已到账。首次购买会生成长期有效 Key；续费后继续使用原 Key。',
    viewMyKeys: '查看我的 Key',
    prepaidPausedStatus: '余额不足，已暂停',
    prepaidPurchaseStatus: '余额不足，待充值',
    ...
    groupChangeConfirm: '切换到“{group}”后，此密钥可用的模型、倍率和扣费来源都会立即变化。确认切换吗？',
    ...
    sharedSubscriptionQuota: '当前订阅共享额度（所有密钥）',
    sharedSubscriptionRemaining: '订阅剩余',
    ...
    useKeyModal: {
      ...
      openai: {
        ...
        authModeDescription: '默认把密钥直接写入 config.toml，保存一个文件即可使用；也可切换为环境变量模式。',
        authModeEnvKey: '环境变量',
        ...
        noteWindows:
          '将 config.toml 保存到 %userprofile%\\.codex 后重启 Codex 即可。默认密钥已在这个文件中；只有选择「环境变量」模式时才需要额外运行命令。'
      },
      cliTabs: {
      ...
      gemini: {
        ...
        modelComment: '可填写当前支持的模型，例如：gemini-3.5-flash',
        note: '这些环境变量将在当前终端会话中生效。如需永久配置，请将其添加到 ~/.bashrc、~/.zshrc 或相应的配置文件中。'
      },
      grok: {
        description:
    ...
    ccsCodex: {
      ...
      importNew: '直接导入 CCS',
      step1: '在 CCS → Codex 中打开原供应商，备份并复制它的完整 config.toml，粘贴到下方生成合并配置。',
      step2: '复制合并结果，在 CCS 原供应商的配置编辑处粘贴并保存；请勿再点击“直接导入 CCS”，否则会新建默认模板。',
      step3: '在 CCS 中切换到已更新的原供应商，再重新打开 Codex；原模型、认证和会话设置会保留。',
      copyEndpoint: '复制接口地址',
      copyKey: '复制 API 密钥',
      preserveHint: '当前 CCS 导入协议会重新生成默认 Codex 参数，无法直接导入并保留原设置。保留模式会合并原 config.toml 的接口和密钥，请将结果粘贴到 CCS 原供应商的配置编辑处；只有选择“不保留，直接导入”才会新建供应商。',
      configLabel: 'CCS 原供应商的完整 config.toml（本地处理）',
      ...
      generate: '保留参数并生成配置',
      copyAndOpenCcs: '复制合并配置，粘贴到 CCS 原供应商',
      mergedCopied: '合并后的 config.toml 已复制，请在 CCS 中粘贴到原供应商',
      copyResult: '仅复制合并配置',
      errors: {
    ...
    rateLimit5hShort: '5小时',
    rateLimit1dShort: '1天',
    rateLimit7dShort: '7天',
    rateLimitUnlimited: '无限制',
    rateLimitResetAt: '{time} 重置',
