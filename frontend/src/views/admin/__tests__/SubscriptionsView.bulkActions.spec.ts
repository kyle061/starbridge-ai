...
describe('subscription bulk operations', () => {
  ...
  it('assigns multiple users once and retries only failed users', async () => {
    ...
    expect(bulkAssign).toHaveBeenCalledWith({ user_ids: [11, 22], group_id: 7, validity_days: 30, quota_usd: 50, usage_multiplier: 6 })
    ...
    await form.trigger('submit')
    await flushPromises()
    expect(bulkAssign).toHaveBeenLastCalledWith({ user_ids: [22], group_id: 7, validity_days: 30, quota_usd: 50, usage_multiplier: 6 })
