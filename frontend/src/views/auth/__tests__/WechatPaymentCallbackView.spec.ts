...
describe('WechatPaymentCallbackView', () => {
  ...
  it('redirects legacy openid callback payloads back to purchase while preserving resume context', async () => {
    locationState.current.hash =
      '#openid=openid-123&state=oauth-state&scope=snsapi_base&payment_type=wxpay_direct&amount=128&order_type=subscription&plan_id=7&quantity=30&redirect=%2Fpayment%3Ffrom%3Dwechat'
